...
std::string GenomicRegion::toString() const {
  ...
  out << chrToString(chr)  << ":" << SnowUtils::AddCommas<int>(pos1) << "-" << SnowUtils::AddCommas<int>(pos2); 
  return out.str();
}

void GenomicRegion::pad(int pad) {
  pos1 = std::max(1, pos1-pad);
  pos2 = std::min(pos2+pad, 250000000); // 2500000000 is dummy for now. should be chr end
}
...
std::string GenomicRegion::chrToString(int ref) {
  std::string ref_id;
  if (ref == 22)
    ref_id = "X";
  else if (ref == 23)
...
void GenomicRegion::random() {
  ...
  uint32_t big;
  SnowUtils::genRandomValue(big, genome_size_XY);
  
  for (size_t k = 0; k < 25; k++)
    if (big < CHR_CLEN[k]) {
      assert(k > 0);
      chr = --k;
      assert(big > CHR_CLEN[chr]);
      pos1 = big - CHR_CLEN[chr];
      pos2 = pos1;
      return;
    }
